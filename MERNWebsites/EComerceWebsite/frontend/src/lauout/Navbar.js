...
export default function Navbar(){
    ...
    return (<>
        <section>
  <div className="py-1 text-center text-sm bg-white border-b border-gray-100">
  ...
  <div className="px-6 lg:px-12 py-6 bg-gray-500">
    <nav className="flex justify-between">
      <div className="flex w-full items-center">
        <a href="/">
          <img className="h-12" src="uinel-assets/logos/ecommerce/uistore-gray-black.svg" alt=""/>
        </a>
        <ul className="hidden xl:flex px-4 ml-20 2xl:ml-40 mr-auto">
           {
                                menuItems.map((item)=>{
                                    if(item.text === "Logout" && loggedInUser){
                                        return <li key={item.text} onClick={handleLogOut} className="ml-3 hover:cursor-pointer">
                                                    {item.text}
                                                </li>
                                    }
                                    return <li key={item.text} className={`${loggedInUser && (item.text === "Login" || item.text === "SignUp")  ? "hidden":item.text === "Logout"?"hidden":""}`}>
                                        <Link 
                                        to={item.to}
                                        className="ml-3 dark:text-white text-base text-gray-800 focus:outline-none  focus:ring-gray-800 hover:underline"
                                        >{item.text}</Link>
                                    </li>
                                })
          } 
        ...
        <div className="relative hidden xl:block">
          <img 
          className="absolute w-10 h-4 top-1/2 transform -translate-y-2/4 pl-6 mt-px" 
          src="https://cdn-icons-png.flaticon.com/512/3917/3917754.png" alt=""/>
          <input className="rounded-3xl py-3 pl-12 pr-5 text-gray-300 font-heading font-medium text-base bg-blue-50 border-2 border-blueGray-100 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 outline-none" style={{width: "180px"}}/>
        </div>
        <div className="hidden md:block flex-shrink-0 w-px h-12 bg-gray-100 ml-12 mr-12"></div>
        {
          loggedInUser? <div className="hidden xl:flex items-center">
          ...
          <Link to="/cart" className="relative inline-block text-gray-400 hover:text-gray-500">
            <div className="absolute bottom-0 right-0 flex items-center justify-center -mb-4 -mr-4 w-6 h-6 text-sm text-white bg-blue-500 rounded-full">{cart?.products.length}</div>
          ...
          <img className="ml-5 w-6 h-6 rounded-full" src="https://www.pngitem.com/pimgs/m/30-307416_profile-icon-png-image-free-download-searchpng-employee.png" alt="mt"/>
